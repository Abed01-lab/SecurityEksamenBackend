...
public class EMF_Creator {
  ...
  private static EntityManagerFactory createEntityManagerFactory(
    ...
    /* Strategy for dev and test
         ...
         */
    String puName = isTest ||
      System.getProperty("IS_INTEGRATION_TEST_WITH_DB") != null
      ? "puTest"
      : "pu"; //Only legal names
    if (puName.equals("puTest")) {
      System.out.println(
        "Using the TEST database via persistence-unit --> puTest "
      );
    } else {
    ...
    try {
      ...
      System.out.println(
        "######      ERROR Creating a persistence Unit       ######"
      );
      System.out.println(
        "###### Have you started the dev and test databases? ######"
      );
      System.out.println(
        "######          (docker-compose up -d )             ######"
      );
